...
const CategoriesPage = () => {
  ...
  const handleFilter = (filters) => {
    fetchCategories({ status: filters.status, keyword: filters.keyword });
  };
  ...
  const handleDelete = async (id) => {
    try {
      const response = await deleteCategory(id);
      if (response.code === 200) {
        setToastMessage(response.message || "Xóa thành công!");
        setToastVariant("success");
        setShowToast(true);
        fetchCategories();
      } else {
  ...
  const handleChangeMulti = async (key, value) => {
    if (!selectedCategories || selectedCategories.length === 0) {
      setToastMessage(
        "Vui lòng chọn ít nhất một danh mục để thực hiện hành động!"
      );
      setToastVariant("warning");
      setShowToast(true);
      return;
      ...
      window.confirm(
        `Bạn có chắc chắn muốn ${
          key === "delete"
            ...
            ? "kích hoạt"
            : "dừng hoạt động"
        } ${selectedCategories.length} danh mục đã chọn không?`
      )
    ) {
      try {
        setMultiLoading(true);
        ...
        if (response.code === 200) {
          ...
          throw new Error(
            response.message || "Lỗi khi thay đổi nhiều danh mục"
          );
        }
      } catch (error) {
        setToastMessage(
          error.message || "Đã xảy ra lỗi khi thay đổi nhiều danh mục!"
        );
        setToastVariant("danger");
        setShowToast(true);
      } finally {
