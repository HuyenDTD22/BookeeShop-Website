...
const EditAccountComponent = ({ account, onSubmit }) => {
  ...
  useEffect(() => {
    if (account) {
      const birthDate = account.birth
        ? new Date(account.birth).toISOString().split("T")[0]
        : "";
      setFormData({
      ...
      setImagePreview(account.avatar || "");
    }
  }, [account]);
  ...
  const handleSubmit = (e) => {
    ...
    formDataToSubmit.append("birth", birthISO);
    ...
    <Card className="shadow-lg border-0">
      <Card.Header className="bg-primary text-white">
        <div className="d-flex align-items-center">
          ...
          <h2 className="m-0 fs-4">Chỉnh sửa tài khoản</h2>
        </div>
      </Card.Header>
